...
| 6       | 06.05 – 12.05 | Nagranie filmiku (3–5 min), iteracja nad raportem                                                                                                                                         |
| 7       | 13.05 – 21.05 | Korekta raportu, weryfikacja deliverables, oddanie projektu                                                                                                                               |
| 8       | 22.05 – 10.06 | Przygotowanie prezentacji finalnej                                                                                                                                                        |
...
Wyniki są niejednoznaczne - częściowo tak, z istotnymi zastrzeżeniami metodologicznymi.

Analiza heatmap i wykresu zbiorczego (`category_attention_barchart.png`) pokazuje, że tokeny kategorii _instrukcja_ (np. „wyjaśnij", „opisz", „proszę") przyciągają uwagę modelu przede wszystkim w pierwszych warstwach lokalnych. W promptach bazowych - które są bardzo krótkie (4–8 tokenów) - token instrukcyjny stanowi znaczny ułamek sekwencji, co automatycznie zawyża jego średnią wagę attention.

W promptach zmodyfikowanych (szczególnie _style_, _framing_, _tone_) do sekwencji dochodzi wiele nowych tokenów (np. „krótko i prosto", „Jako ekonomista", „Dla ucznia 5. klasy"). Nowe tokeny modyfikatora też funkcjonują jako „instrukcja" dla modelu, ale ich wpływ trudno odczytać bezpośrednio z diff heatmap: macierz różnicowa operuje na pozycjach `min(seq_len_base, seq_len_mod)` i fizycznie nie obejmuje nowo dodanych tokenów modyfikatora (np. dla `style_01` diff to macierz 9×9 obejmująca jedynie tokeny base „Wyjaśnij grawitację", bez „krótko i prosto"). W obrębie wspólnych pozycji obserwujemy jedynie subtelną redystrybucję attention (skala zmian rzędu 0,001–0,003 wobec średniej attention ~0,05). Główna ilościowa konsekwencja modyfikacji - wzrost entropii Shannona - wynika z faktu, że attention musi pokryć dłuższą sekwencję.
...
Warstwy lokalne (sliding window) reagują silniej na modyfikacje promptów niż warstwy z globalnym attention - we wszystkich pięciu kategoriach (tabela poniżej).
...
To ma bezpośrednie przełożenie na attention: tokeny-instrukcje w języku polskim są często rozbite na wiele subtokenów, co może rozmywać sygnał attention przypisany do „funkcji instrukcji" jako całości. Ręczna kategoryzacja tokenów jako `instrukcja`/`treść`/`funkcyjny` działa poprawnie dla całych słów, ale nie uwzględnia tego, że jeden konstrukt semantyczny może być rozłożony na kilka pozycji.
...
Tokeny treści bazowej pozostają na tych samych pozycjach, a 6 dodanych tokenów stylistycznych jest dopisanych na końcu sekwencji. Dane `source_attributions` z Inseq są zapisane dla obu wersji promptu.

---

### Wnioski końcowe

1. Tokeny instrukcji vs treści - wynik niejednoznaczny (RQ1). Analiza wykazała wysokie wagi attention na tokenach instrukcyjnych w promptach bazowych, ale jest to w dużej mierze artefakt ich dominacji w krótkich sekwencjach (4–8 tokenów) - token „wyjaśnij" stanowi 1/8 sekwencji, więc otrzymuje średnio 12,5% wagi nawet przy rozkładzie jednorodnym. Po dodaniu modyfikatora („instrukcji" dodatkowej) uwaga rozkłada się szerzej i entropia rośnie. Ręczna kategoryzacja oparta na słowach kluczowych i subtokenizacja SentencePiece dodatkowo utrudniają jednoznaczne rozstrzygnięcie. Wyniki są spójne z literaturą (Clark et al. 2019) wskazującą, że tokeny funkcyjne często otrzymują wysokie wagi niezwiązane z ich semantyczną rolą.

2. Wzrost entropii attention wystąpił po modyfikacji we wszystkich 25 parach promptów; średnia wartość ΔE = 0,34. Mechanizm jest prosty: dłuższe i bardziej opisowe prompty wymagają od modelu rozproszenia uwagi na większą liczbę tokenów.

3. Sparsity rośnie wraz z modyfikacją, z jednym wyjątkiem - `reformulation_02` wykazuje nieznaczny spadek sparsity. Wzrost sparsity oznacza, że zmodyfikowane prompty prowadzą do bardziej skoncentrowanych rozkładów attention - pozornie sprzeczne z wzrostem entropii, ale wyjaśnialne przez dłuższe sekwencje: więcej tokenów z bardzo niską wagą (< 0,01) podnosi odsetek sparsity, a jednocześnie więcej tokenów z umiarkowaną wagą podnosi entropię Shannona.

4. Diff L1/L2 jest nierzetelną miarą przy dużych różnicach długości sekwencji - a dla par z prefix modifiers jest dodatkowo niesemantyczna. Kategorie _style_ i niektóre pary _framing_ mają diff L1 ≈ 0,0001 - 100× niższy niż inne kategorie z podobnymi zmianami entropii. Wynika to z faktu, że `pairwise_attention_diff` operuje na obciętej macierzy o rozmiarze `min(seq_len_base, seq_len_mod)`, co przy promptach bazowych liczących 4–8 tokenów i zmodyfikowanych 14–26 tokenów drastycznie redukuje obszar porównania. Dla par z modyfikatorem na początku promptu (_framing_, _formality_ z „Proszę o…", _tone_ z „Jako ekonomista,…") problem jest poważniejszy: diff porównuje attention między pozycjami, które w base i mod reprezentują różne tokeny semantycznie. Np. w `framing_04` komórka diff `[?, <bos>]` o wartości +0,13 wynika z tego, że pozycje 0–4 w modified to subtokeny słowa „Wyjaśnij" (`['<bos>','Wy','ja','ś','nij']`), a w base - `['<bos>','Jak','▁działa','▁komputer','?']`. Wartości diff dla takich par są niesemantyczne i powinny być interpretowane wyłącznie jako pomiar zmiany rozkładu attention na pozycjach, nie na konkretnych tokenach. Metryka diff działa rzetelnie tylko przy zbliżonych długościach sekwencji i zachowanej kolejności tokenów wspólnych.

5. Warstwy lokalne (sliding window) są bardziej wrażliwe na modyfikacje niż globalne we wszystkich kategoriach. Potwierdza to hipotezę, że zmiany w bezpośrednim sąsiedztwie tokenu (dodanie modyfikatora na końcu krótkiego promptu) są silniej uchwycone przez mechanizm lokalnego attention.

6. Framing wykazuje największy i najbardziej zróżnicowany efekt. Szczegółowy opis odbiorcy tworzy najsilniejszą redystrybucję uwagi - model musi „wziąć pod uwagę" wiele dodatkowych informacji kontekstowych. Największy efekt: `framing_04` - opis „osoby po 70. roku życia, która nigdy nie korzystała z technologii".

7. Gemma 3 4B nie różnicuje istotnie wzorców attention między „Co powoduje deszcz?" a „Dlaczego pada deszcz?" - ΔE = 0,0219, najniższa wartość w całym eksperymencie. Parafraza semantycznie równoważna o podobnej strukturze składniowej nie zmienia rozkładu uwagi.
